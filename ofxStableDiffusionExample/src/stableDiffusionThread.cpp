...
void stableDiffusionThread::threadedFunction() {
	...
	if (thread->isModelLoading) {
		...
		sd_ctx = new_sd_ctx(&thread->modelPath[0], &thread->vaePath[0],  &thread->taesdPath[0], &thread->controlNetPath[0], &thread->loraModelDir[0], &thread->embedDir[0], thread->isVaeDecodeOnly, thread->isVaeTiling, thread->isFreeParamsImmediatly, thread->numThreads, thread->sdType, thread->rngType, thread->schedule, false);
		...
		thread->output_images = txt2img(sd_ctx, &thread->prompt[0], &thread->negativePrompt[0], thread->clipSkipLayers, thread->cfgScale, thread->width, thread->height, thread->sampleMethodEnum, thread->sampleSteps, thread->seed, thread->batchSize, thread->control_image, thread->controlStrength);
		if (thread->isESRGAN) {
